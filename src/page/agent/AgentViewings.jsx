...
export default function AgentViewings() {
  const { fetchAgentViewings, confirmViewing, declineViewing, cancelViewing, completeViewing } =
  ...
  const loadViewings = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await fetchAgentViewings(statusFilter ? { status: statusFilter } : {});
      setViewings(data);
    } catch (err) {
      console.error(err);
  ...
  }, [fetchAgentViewings, statusFilter]);
  ...
  const runAction = async (id, action, extraArg) => {
    setActionId(id);
    try {
      const updated = await action(id, extraArg);
  ...
  const handleConfirm = (id) => runAction(id, confirmViewing).then(() => toast.success("Viewing confirmed."));
  const handleDecline = (id) => {
    const reason = window.prompt("Reason for declining this viewing (optional):") || "";
    runAction(id, declineViewing, reason).then(() => toast.success("Viewing declined."));
  };
  const handleCancel = (id) => {
    const reason = window.prompt("Reason for cancelling this viewing (optional):") || "";
    runAction(id, cancelViewing, reason).then(() => toast.success("Viewing cancelled."));
  };
  const handleComplete = (id) => runAction(id, completeViewing).then(() => toast.success("Viewing marked as completed."));
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-4xl border border-dashed border-slate-300 bg-white/60 p-10 text-center text-sm text-slate-500">
          No viewing requests yet. When a tenant books a viewing for one of your listings, it will appear here.
        </div>
      ) : (
        <div className="space-y-4">
          {viewings.map((viewing) => (
            <div key={viewing._id} className="rounded-4xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-semibold text-slate-900">{viewing.listing?.title || "Listing"}</p>
                  <p className="mt-1 flex items-center gap-2 text-sm text-slate-500">
                    <User2 size={14} /> {viewing.tenant?.firstName} {viewing.tenant?.lastName}
                  </p>
                  {viewing.tenant?.phone ? (
                    <p className="mt-1 flex items-center gap-2 text-xs text-slate-400">
                      <Phone size={12} /> {viewing.tenant.phone}
                    </p>
                  ) : null}
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {viewing.status === "requested" ? (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => handleDecline(viewing._id)}
                      disabled={actionId === viewing._id}
